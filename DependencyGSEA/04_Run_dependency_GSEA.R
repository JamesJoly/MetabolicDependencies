...
gene.set.names <- all_pathways$geneset.names
gene.set.names <- paste(gene.set.names, "Dependency", sep = " ")
all_pathways$geneset.names <- gene.set.names

colnames(adherent.rpmi) <- paste(colnames(adherent.rpmi),"Correlation", sep = " ")
colnames(adherent.rpmi)[1] <- "Gene"

colnames(adherent.dmem) <- paste(colnames(adherent.dmem),"Correlation", sep = " ")
colnames(adherent.dmem)[1] <- "Gene"
...
### Make GSEA mountain plots as in Fig 2 and Supp Fig 1
GSEA_preranked <- function(input.df, gmt.list,
                        num.permutations = 1000,
                        stat.type = "Weighted"){
  nperm = num.permutations #number of permutations
  if (stat.type == "Classic"){
...
adherent.rpmi.TCA <- dplyr::select(adherent.rpmi,  matches("Gene"), contains("KEGG_CITRATE_CYCLE_TCA_CYCLE"), -contains("Glycolysis"), -contains("Oxidative"))
adherent.rpmi.TCA.gsea <- GSEA_preranked(input.df = adherent.rpmi.TCA, gmt.list = all_pathways)

adherent.dmem.TCA <- dplyr::select(adherent.dmem,  matches("Gene"), contains("KEGG_CITRATE_CYCLE_TCA_CYCLE"), -contains("Glycolysis"), -contains("Oxidative"))
adherent.dmem.TCA.gsea <- GSEA_preranked(input.df = adherent.dmem.TCA, gmt.list = all_pathways)
...
oneC <- "KEGG_ONE_CARBON_POOL_BY_FOLATE Dependency"
TCA.1C.dmem <- gsea_mountain_plot(GSEA.list = adherent.dmem.TCA.gsea, Sample.Name = "KEGG_CITRATE_CYCLE_TCA_CYCLE Correlation",
                             Gene.Set.A = oneC)
ggplot2::ggsave(TCA.1C.dmem, file = "one carbon Dependency upon tca Expression adherent dmem.pdf", height = 4, width = 4)

TCA.1C.rpmi <- gsea_mountain_plot(GSEA.list = adherent.rpmi.TCA.gsea, Sample.Name = "KEGG_CITRATE_CYCLE_TCA_CYCLE Correlation",
                             Gene.Set.A = oneC)
ggplot2::ggsave(TCA.1C.rpmi, file = "one carbon Dependency upon tca Expression adherent rpmi.pdf", height = 4, width = 4)
